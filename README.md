...
| Склад и журнал движений | http://localhost:8080/admin/inventory |

---

## Склад, CSV и публикация

- На странице `/admin/inventory` показаны уведомления о низком и нулевом остатке, быстрые формы цены/количества и журнал движений.
- Экспорт CSV служит актуальным шаблоном импорта. Файл кодируется в UTF-8 и содержит товары и варианты; строки с `product_id`/`variant_id` обновляются, пустой `product_id` создаёт товар.
- Причина ручной корректировки сохраняется в журнале вместе с автором, старым и новым количеством. Резервирование заказа и возврат резерва также записываются автоматически.
- Товары и акции поддерживают статусы `DRAFT`, `SCHEDULED`, `PUBLISHED`. Для `SCHEDULED` обязательно поле `publish_at`; фоновая проверка выполняется каждые 30 секунд (настраивается `APP_PUBLICATION_CHECK_MS`).
